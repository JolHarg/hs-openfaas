...
runCommand "openfaas" {
    ...
    # 
    shellHook = ''
        # x86_64-unknown-linux-gnu-ghc src/Main.hs -ilib -o packages/sample/hello/openfaas
        # rm src/Main src/Main.{hi,o}

        #cabal new-build

        #cp $(cabal exec which openfaas) packages/sample/hello/openfaas
        #x86_64-unknown-linux-gnu-strip packages/sample/hello/openfaas
        #patchelf --set-interpreter /lib64/ld-linux-x86-64.so.2 packages/sample/hello/openfaas
